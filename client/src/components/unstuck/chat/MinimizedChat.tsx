...
export const MinimizedChat = ({ isWorkflowActive, latestMessage, error, loading, onMaximize }: MinimizedChatProps) => (
  <Card 
    onClick={onMaximize}
    className={`
      ...
      {loading?.isLoading && (
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {loading.progress !== undefined && (
            <span className="text-xs text-purple-500">{Math.round(loading.progress * 100)}%</span>
          )}
        </div>
      )}
    </div>
